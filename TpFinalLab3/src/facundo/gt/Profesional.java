...
public class Profesional extends Usuario implements CrearTratamiento {
  ...
  -VerDatosPaciente(atributos, historial, etc)*/
    ...
    public void VerInfoPacienteDeterminado() {
        String dni;
    ...
    @Override
    public PlanDeControl crearTratamiento(HashMap<String, Paciente> pacs, ArrayList<PlanDeControl> p) {
        ...
        pacientesAAtender.remove(listaConver.get(0));
          ...
          /* System.out.println("desea mandar una peticion para predeterminar este plan?s/n");
            char rta = scan.next().charAt(0);
            if (rta == 's' || rta == 'S') {
                peticionDePredeterminar(plan);
            }
            /*else{
                try {
                    throw new MyException(rta);
                } catch (MyException e) {
                    e.printStackTrace();
                }
            }*/
    ...
    /*static void peticionDePredeterminar(PlanDeControl p){
        System.out.println("escriba brevemente su peticion a la administracion");
        String txt = "";
        txt = sc.nextLine();
    }*/
